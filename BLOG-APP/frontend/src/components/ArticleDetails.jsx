...
function ArticleDetails() {
  ...
  useEffect(() => {
    ...
    const getArticle = async () => {
      ...
      try {
        const res = await axios.get(`${process.env.VITE_API_URL}/user-api/articles/${id}`, { withCredentials: true });
  ...
  const toggleArticleStatus = async () => {
    ...
    try {
      const res = await axios.patch(
        `${process.env.VITE_API_URL}/author-api/articles`,
  ...
  const addComment = async (commentObj) => {
    ...
    let res = await axios.put(`${process.env.VITE_API_URL}/user-api/articles`, commentObj, { withCredentials: true });
    if (res.status === 200) {
